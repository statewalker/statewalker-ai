...
export class AgentRuntimeManager {
  ...
  private async _runRebuild(): Promise<void> {
    ...
    if (!active) {
      // The "no-providers" vs "no-active-model" distinction is
      // owned by the providers fragment (Wave 4.2; an interim
      // bootstrap stands in until then). The manager only writes
      // `ready` / `error` states; if there's no active model, the
      // current adapter state is left untouched.
      return;
    }
