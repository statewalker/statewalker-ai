...
import { LocalLanguageModel } from "../../src/transformers/language-model.js";

function createMockPipeline(response = "Hello back!") {
  const tokenizer = {
    apply_chat_template: vi.fn((_messages: unknown[], _opts: unknown) => "formatted prompt"),
  };
...
describe("LocalLanguageModel", () => {
  describe("properties", () => {
    it("has correct specificationVersion, provider, modelId", () => {
      const model = new LocalLanguageModel("test/model", createMockPipeline(), createMockTjs());
  ...
  describe("doGenerate", () => {
    it("returns text content, finishReason, and usage", async () => {
      ...
      const model = new LocalLanguageModel("test/model", pipeline, createMockTjs());
  ...
  describe("doStream", () => {
    it("produces a readable stream with text-start, text-delta, text-end, finish", async () => {
      ...
      const pipeline = Object.assign(
        vi.fn(async (_prompt: string, opts: { streamer: { _callback: (t: string) => void } }) => {
