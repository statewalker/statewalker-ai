...
export class LlamaCppLanguageModel implements LanguageModelV3 {
  ...
  async doGenerate(
    ...
    const { systemPrompt, history, lastUserText } = convertPrompt(
      options.prompt,
    );
    ...
    if (options.maxOutputTokens != null)
      promptOpts.maxTokens = options.maxOutputTokens;
  ...
  async doStream(
    ...
    const { readable, writable } =
      new TransformStream<LanguageModelV3StreamPart>();
    ...
    (async () => {
      ...
      try {
        ...
        await writer.write({
          type: "finish",
          finishReason: options.abortSignal?.aborted
            ? unified("other")
            : unified("stop"),
