...
describe("resolveGgufFiles", () => {
  it("returns a single-file list from config", async () => {
    ...
    expect(out).toEqual([{ name: BASE_CONFIG.ggufFile, size: BASE_CONFIG.sizeBytes }]);
  });
...
describe("verifyGgufWeights", () => {
  ...
  it("returns true when a .gguf file is present", async () => {
    expect(await verifyGgufWeights(iter([{ kind: "file", name: "Llama.Q4_K_M.gguf" }]))).toBe(true);
  });

  it("returns false when no .gguf file is present", async () => {
    expect(await verifyGgufWeights(iter([{ kind: "file", name: "readme.md" }]))).toBe(false);
  });
