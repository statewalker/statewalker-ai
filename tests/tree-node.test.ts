...
import type { NodeRegistry, TreeEntry } from "../src/types.js";
...
describe("TreeNode construction", () => {
  it("wraps a TreeEntry with Snowflake ID", () => {
    ...
    const node = new TreeNode(data, emptyRegistry);
...
describe("parentId", () => {
  it("returns parent id when attached", () => {
    const parent = new TreeNode(entry("session"), emptyRegistry);
    const child = parent.addChild(entry("turn"));
    expect(child.parentId).toBe(parent.id);
  });

  it("returns undefined for root", () => {
    const root = new TreeNode(entry("session"), emptyRegistry);
...
describe("createdAt / updatedAt / touch", () => {
  ...
  it("touch sets props.updatedAt as ISO string", () => {
    const node = new TreeNode(entry("test"), emptyRegistry);
    node.touch();
...
describe("children (cached wrappers)", () => {
  ...
  it("uses registry to create typed children", () => {
    const registry: NodeRegistry = new Map();
    class MyTurn extends TreeNode {}
    registry.set("turn", (d, r) => new MyTurn(d, r));

    const parent = new TreeNode(entry("session"), registry);
    const child = parent.addChild(entry("turn"));
...
describe("visit", () => {
  it("traverses depth-first", () => {
    const root = new TreeNode(entry("session", { id: "root" }), emptyRegistry);
...
describe("childrenOfType", () => {
  it("filters by props.type", () => {
    const parent = new TreeNode(entry("turn"), emptyRegistry);
